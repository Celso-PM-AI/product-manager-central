...
is stored locally in SQLite. Phase 9 Checkpoints 1 through 3 add an optional,
inactive-by-default OpenAI connection, semantic retrieval from Approved BRDs
and PRDs, and temporary grounded draft generation with citations.
...
- An AI Assistant that returns temporary generated drafts grounded in retrieved
  Approved content, with structured citations and no automatic saving.
...
Phase 9 Checkpoints 1 through 3 provide the secure connection, approved-source
retrieval, and grounded generated-draft workflow. The AI Assistant is available
...
Generated drafts are temporary and separate from original BRDs and PRDs. The
assistant never modifies, overwrites, or appends to a source document. Checkpoint
3 has no save action: a human must review and explicitly accept generated
content before the later Checkpoint 4 workflow can save it.
...
- Human review, explicit acceptance, and separate storage for AI-generated
  content; automatic changes to source BRDs/PRDs remain prohibited
...
temporary grounded draft generation with citations are implemented.
Checkpoints 4, 5, and 6 are not started. No generated-content saving or
acceptance workflow, prompt management, or RAG evaluation is included yet.
